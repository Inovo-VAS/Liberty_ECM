...
namespace InovoCIM.FileProcess
{
    public class FileTSFReferrals
    {
        ...
        public FileTSFReferrals(string _InstanceID, ImportFile _ImportFileModel, FileInfo _File)
        {
        ...
        public async Task<bool> Master()
        {
            ...
            try
            {
                ...
                Table = TableRepo.ColumnTableTSFReferrals(Table);
        ...
        private async Task<string> SendToDatabase()
        {
            ...
            try
            {
                ...
                {
                    int result = Sql.SendToInovoCIM(Table, "[ECM].[FileTableTSFReferrals]"); 
                }
        ...
        private async Task<string> MapFilePresence  (string[] InData, int SourceID, int ServiceID, int LoadID, PhoneModel Phones)
        {
            try
            {
                ...
                if(!String.IsNullOrEmpty(InData[4]))
                {
                    try
                    {
                        CultureInfo provider = new CultureInfo("fr-FR");
                        Schedule = DateTime.ParseExact(InData[4], "g", provider);
                        Row["SCHEDULEDATE"] = Schedule;
                    }
                    catch
                    {
                        Row["SCHEDULEDATE"] = DBNull.Value;
                ...
                Row["PRIORITY"] = 1;
                int? CapturingAgent = null;
                if (Schedule != null)
                {
                    CapturingAgent = Convert.ToInt32(InData[3]);
                    Row["CAPTURINGAGENT"] = CapturingAgent.ToString();
                }
                else
                {
