...
namespace InovoCIM.FileProcess
{
    public class FileRecoveries
    {
        ...
        public int IndexServiceIndicator { get; set; }
        ...
        public FileRecoveries(string _InstanceID, ImportFile _ImportFileModel, FileInfo _File)
        {
            ...
            this.IndexLoadIndicator = 8;
            this.IndexServiceIndicator = 7;
        ...
        public async Task<bool> Master()
        {
            ...
            try
            {
                ...
                Table = TableRepo.ColumnTableRecoveriesSMS(Table);
        ...
        private async Task<string> ValidateHeader()
        {
            ...
            try
            {
                ...
                //string header = File.ReadLines(Path.Combine("C:\\0.InovoCIM\\Busy", this.FileDetail.Name)).First();
        ...
        private async Task<string> ReadTextFile()
        {
            ...
            try
            {
                ...
                //using (var fileStream = File.OpenRead(Path.Combine("C:\\0.InovoCIM\\Busy", this.FileDetail.Name)))
                using (var reader = new StreamReader(fileStream, Encoding.UTF8, true, 1024))
                {
                    while ((fileLine = reader.ReadLine()) != null)
                    {
                        if (fileLine != ImportFileModel.Header)
                        {
                            ...
                            var temp = InData[this.IndexServiceIndicator];
                            int ServiceID = int.Parse(InData[this.IndexServiceIndicator]); // CHECK THIS
                            int SourceID = int.Parse(InData[this.IndexSourceID]);
                            int LoadID = await CalculateLoadID(InData, ServiceID, this.FileDetail.Name);
        ...
        private async Task<string> SendToDatabase()
        {
            ...
            try
            {
                ...
                {
                    int result = Sql.SendToInovoCIM(Table, "[ECM].[FileTableRecoveriesSMS]"); 
                }
        ...
        public async Task<int> CalculateLoadID(string[] InData, int ServiceID, string FileName)
        {
            int LoadID = 0;
            try
            {
                ...
                bool InIndicatorList = IndicatorList.ContainsValue(FileName);
                if (InIndicatorList == true)
                {
                    LoadModel Load = new LoadModel(this.InstanceID, ServiceID, DateTime.Now.ToString("ddMMyyyy"), FileName, "Date");
                    LoadID = await Load.GetLoadID();
                }
                else
                {
                    IndicatorList.Add(this.StatsLineTotal, FileName);
                    LoadModel Load = new LoadModel(this.InstanceID, ServiceID, DateTime.Now.ToString("ddMMyyyy"), FileName, "Date");
        ...
        private async Task<string> MapFileLog(string[] InData, int FileLine, int ServiceID, int LoadID, int SourceID, string IsFileDuplicate, string IsPresenceDuplicate, string IsScriptDuplicate, string MapPresenceFail, string MapScriptFail, string PhoneStatus)
        {
            try
            {
                ...
                Row["ProductName_&_StatusDate"] = InData[4]; //Source_ID;Name;IDNumber;CellNumber;ProductName_&_StatusDate;Premium;DOB;SERVICEID
                Row["Premium"] = InData[5];
                Row["DOB"] = InData[6];
                Row["SERVICEID"] = InData[7];
                Table.Rows.Add(Row);
                return "Ok";
            }
            catch (Exception ex)
        ...
        private async Task<string> MapFilePresence  (string[] InData, int SourceID, int ServiceID, int LoadID, PhoneModel Phones)
        {
            try
            {
                string ConcatName = InData[1] + " " + InData[6];
                ConcatName = ConcatName.Length <= 40 ? ConcatName : ConcatName.Substring(0, 40);
