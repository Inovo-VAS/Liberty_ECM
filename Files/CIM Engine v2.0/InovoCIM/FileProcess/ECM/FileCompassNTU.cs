...
namespace InovoCIM.FileProcess
{
    public class FileCompassNTU
    {
        ...
        public DataTable Table_AVM = new DataTable();
        public DataTable TableScript = new DataTable();
        public DataTable Presence = new DataTable();
        public DataTable Presence_AVM = new DataTable();
        public DataTable QueueComplete = new DataTable();
        public DataTable QueueComplete_AVM = new DataTable();

        public Dictionary<int, string> IndicatorList = new Dictionary<int, string>();

        #region [ Default Constructor ]
        public FileCompassNTU(string _InstanceID, ImportFile _ImportFileModel, FileInfo _File)
        {
        ...
        public async Task<bool> Master()
        {
            ...
            try
            {
                ...
                Table = TableRepo.ColumnTableECMNTUPDATE(Table);
        ...
        #region [ Read Text File ]
        private async Task<string> ReadTextFile_AVM()
        {
            string LeadStatus = "Success";
            Dictionary<int, int> SourceList = new Dictionary<int, int>();
            try
            {
                ...
                using (var reader = new StreamReader(fileStream, Encoding.UTF8, true, 1024))
                {
                    while ((fileLine = reader.ReadLine()) != null)
                    {
                        ...
                        {
                            this.StatsLineTotal++;
                            string[] InData = fileLine.Split(',').ToArray();

                            int ServiceID = 454; // CHECK THIS
                            int SourceID = int.Parse(InData[this.IndexSourceID]);
                            int LoadID = await CalculateLoadID(InData, ServiceID);
                            ...
                            if (IsFileDuplicate == false)
                            {
                                ...
                                if (IsPresenceDuplicate == false)
                                {
                                    PhoneModel Phones = new PhoneModel(this.InstanceID, InData[this.IndexPhone01_AVM], InData[this.IndexPhone01_AVM], InData[this.IndexPhone01_AVM]);
                                    //Phones = await Phones.GetPhoneNumbers();
                                    _PhoneStatus = "Valid Numbers";
                                    if (_PhoneStatus == "Valid Numbers")
                                    {
                                        string QueuePresence = await MapFilePresence_AVM(InData, SourceID, ServiceID, LoadID, Phones);
                                        if (QueuePresence == "Fail")
                                        {
                                            LeadStatus = "Map Presence Failed";
                                            _MapPresenceFail = "Yes";
                            ...
                            string Log = await MapFileLog_AVM(InData, this.StatsLineTotal, ServiceID, LoadID, SourceID, _IsFileDuplicate, _IsPresenceDuplicate, _IsScriptDuplicate, _MapPresenceFail, _MapScriptFail, _PhoneStatus);
                            string QueueComplete = await MapFileQueueComplete_AVM(InData, LeadStatus, SourceID, ServiceID, LoadID);
                        }
                    }
                }
        ...
        private async Task<string> SendToDatabase()
        {
            ...
            try
            {
                ...
                {
                    int result = Sql.SendToInovoCIM(Table, "[ECM].[FileTableCompassNTU]"); 
                }
        ...
        #region [ Send To Database AVM ]
        private async Task<string> SendToDatabase_AVM()
        {
            SqlBulkRepository Sql = new SqlBulkRepository();
            try
            {
                if (Table_AVM.Rows.Count > 0)
                {
                    int result = Sql.SendToInovoCIM(Table_AVM, "[ECM].[FileTableBMIAVM]");
                }
                ...
                if (Presence_AVM.Rows.Count > 0)
                {
                    int result = Sql.SendToPresence(Presence_AVM, "[PREP].[PCO_OUTBOUNDQUEUE]");
                }

                if (QueueComplete_AVM.Rows.Count > 0)
                {
                    int result = Sql.SendToInovoCIM(QueueComplete_AVM, "[ECM].[QueuePhoneComplete]");
                }
        ...
        private async Task<string> MapFileLog(string[] InData, int FileLine, int ServiceID, int LoadID, int SourceID, string IsFileDuplicate, string IsPresenceDuplicate, string IsScriptDuplicate, string MapPresenceFail, string MapScriptFail, string PhoneStatus)
        {
            try
            {
                ...
                Row["ID_NUMBER"] = InData[1];
                Row["SURNAME"] = InData[2];
                Row["NAME"] = InData[3];
                Row["CELL_NUMBER"] = InData[4];
                Row["ID_NUMBER"] = InData[5];
                Table.Rows.Add(Row);
                return "Ok";
            }
            catch (Exception ex)
        ...
        #region [ Map File Log AVM ]
        private async Task<string> MapFileLog_AVM(string[] InData, int FileLine, int ServiceID, int LoadID, int SourceID, string IsFileDuplicate, string IsPresenceDuplicate, string IsScriptDuplicate, string MapPresenceFail, string MapScriptFail, string PhoneStatus)
        {
            try
            {
                FileLine++;
        ...
        #region [ Map File Queue Complete AVM ]
        private async Task<string> MapFileQueueComplete_AVM(string[] InData, string Status, int SourceID, int ServiceID, int LoadID)
        {
            try
            {
                string Name = InData[1];
                Name = InData[1].Length <= 40 ? InData[1] : InData[1].Substring(0, 40);
                DataRow Row = QueueComplete_AVM.NewRow();
                Row["QueuePhoneCompleteID"] = DBNull.Value;
                Row["Command"] = "AddCall";
                Row["Input"] = "File Import";
                Row["InputName"] = this.FileDetail.Name;
                ...
                Row["CallerID"] = DBNull.Value;
                Row["CallerName"] = DBNull.Value;
                QueueComplete_AVM.Rows.Add(Row);
        ...
        #region [ Map File Presence AVM ]
        private async Task<string> MapFilePresence_AVM(string[] InData, int SourceID, int ServiceID, int LoadID, PhoneModel Phones)
        {
            try
            {
                string Name = InData[1];
                Name = InData[1].Length <= 40 ? InData[1] : InData[1].Substring(0, 40);
                DataRow Row = Presence_AVM.NewRow();
                Row["ID"] = DBNull.Value;
                Row["SERVICEID"] = ServiceID;
                Row["NAME"] = Name;
                Row["PHONE"] = Phones.PH01;
