...
namespace InovoCIM.FileProcess
{
    public class FileQLINKLapse
    {
        ...
        public FileQLINKLapse(string _InstanceID, ImportFile _ImportFileModel, FileInfo _File)
        {
        ...
        public async Task<bool> Master()
        {
            ...
            try
            {
                ...
                Table = TableRepo.ColumnTableQLINKLAPSE(Table);
        ...
        private async Task<string> ReadTextFile_AVM()
        {
            ...
            try
            {
                ...
                using (var reader = new StreamReader(fileStream, Encoding.UTF8, true, 1024))
                {
                    while ((fileLine = reader.ReadLine()) != null)
                    {
                        if (fileLine != "Source_ID,Name,IDNumber,CellNumber")
                        {
                            this.StatsLineTotal++;
                            string[] InData = fileLine.Split(',').ToArray();

                            int ServiceID = 564; // CHECK THIS
                            int SourceID = int.Parse(InData[this.IndexSourceID]);
                            int LoadID = await CalculateLoadID(InData, ServiceID);
        ...
        private async Task<string> SendToDatabase()
        {
            ...
            try
            {
                ...
                {
                    int result = Sql.SendToInovoCIM(Table, "[ECM].[FileTableQlinkLapse]"); 
                }
