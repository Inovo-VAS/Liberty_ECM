...
namespace InovoCIM.FileProcess
{
    public class FileStandard
    {
        ...
        public FileStandard(string _InstanceID, ImportFile _ImportFileModel, FileInfo _File)
        {
            ...
            this.IndexSourceID = 5;
            this.IndexLoadIndicator = 7;
        ...
        public async Task<bool> Master()
        {
            ...
            try
            {
                ...
                Table = TableRepo.ColumnTableStandard(Table);
        ...
        private async Task<string> ReadTextFile()
        {
            ...
            try
            {
                ...
                int ServiceID = 0;
                switch (this.ImportFileModel.Partial)
                {
                    case "CLEVVA1":
                        ServiceID = 1802;
                        break;
                ...
                using (var reader = new StreamReader(fileStream, Encoding.UTF8, true, 1024))
                {
                    while ((fileLine = reader.ReadLine()) != null)
                    {
                        ...
                        {
                            this.StatsLineTotal++;
                            string[] InData = fileLine.Split(this.ImportFileModel.Delimiter).ToArray();
                            ...
                            string _IsFileDuplicate = "No", _IsPresenceDuplicate = "No", _IsScriptDuplicate = "N/A", _MapPresenceFail = "No", _MapScriptFail = "N/A";
        ...
        public async Task<int> CalculateLoadID(string[] InData, int ServiceID)
        {
            ...
            try
            {
                string LoadPartialName = "";
                string FullPartial = FileDetail.Name.Substring(0, 14);
                switch (FullPartial)
                {
                    case "CC2_HR_SBBASE_":
                        LoadPartialName = "CC2_HR_SBBASE";
                        break;
                    case "CC2_GOLD_SBBAS":
                        LoadPartialName = "CC2_GOLD_SBBASE";
                        break;
                    case "CC2_HR_CONTING":
                        LoadPartialName = "CC2_HR_CONTINGENCY";
                        break;
                    case "CC2_GOLD_CONTI":
                        LoadPartialName = "CC2_GOLD_CONTINGENCY";
                        break;
                    case "CC1_HR_SBBASE_":
                        LoadPartialName = "CC1_HR_SBBASE";
                        break;
                    case "CC1_HR_CONTING":
                        LoadPartialName = "CC1_HR_CONTINGENCY";
                        break;
                    case "CC1_GOLD_SBBAS":
                        LoadPartialName = "CC1_GOLD_SBBASE";
                        break;
                    case "CC1_GOLD_CONTI":
                        LoadPartialName = "CC1_GOLD_CONTINGENCY";
                        break;
                    default:
                        LoadPartialName = "CC2_HR_SBBASE";
                        break;
                ...
                if (this.ImportFileModel.Partial == "CLEVVA1" || this.ImportFileModel.Partial == "CLEVVA2" || this.ImportFileModel.Partial == "Retrofit")
                {
                    LoadPartialName = this.ImportFileModel.Partial;
                }

                if (this.ImportFileModel.Partial == "Win")
                {
                    LoadPartialName = "Win Back";
                }

                if (this.ImportFileModel.Partial == "Cross")
                {
                    LoadPartialName = "Cross & Upsell";
                }
                ...
                {
                    LoadModel Load = new LoadModel(this.InstanceID, ServiceID, InData[this.IndexLoadIndicator], LoadPartialName, "Batch");
        ...
        private async Task<string> SendToDatabase()
        {
            ...
            try
            {
                ...
                {
                    int result = Sql.SendToInovoCIM(Table, "[ECM].[FileTableStandard]");
                }
        ...
        private async Task<string> MapFileLog(string[] InData, int FileLine, int ServiceID, int LoadID, int SourceID, string IsFileDuplicate, string IsPresenceDuplicate, string IsScriptDuplicate, string MapPresenceFail, string MapScriptFail, string PhoneStatus)
        {
            try
            {
                ...
                Row["Input"] = (this.FileDetail.Name.Length <= 119) ? this.FileDetail.Name : this.FileDetail.Name.Substring(0, 119);
                ...
                Row["Name"] = (InData[0].Length <= 249) ? InData[0] : InData[0].Substring(0,249);
                Row["IDN"] = (InData[1].Length <= 249) ? InData[1] : InData[1].Substring(0, 249);
                Row["Mobile"] = InData[2];
                Row["HomePhone"] = InData[3];
                Row["WorkPhone"] = InData[4];
                Row["SourceID"] = InData[5];
                Row["SourceCampaign"] = InData[6];
                Row["Batch"] = InData[7];
                Table.Rows.Add(Row);
        ...
        private async Task<string> MapFileQueueComplete(string[] InData, string Status, int SourceID, int ServiceID, int LoadID)
        {
            try
            {
                string name = InData[0].Length <= 39 ? InData[0] : InData[0].Substring(0, 39);
                ...
                Row["Phone02"] = InData[3];
                Row["Phone03"] = InData[4];
